-- ============================================================
-- Moment Weaver · 心声信箱 (Heartcove) Schema（开发者参考副本）
-- 适用：MySQL 8.0+
-- 库名：moment_weaver
--
-- ⚠️ 正常情况下不需要手动执行本脚本！
-- 权威迁移脚本在 Flyway：
--   backend/moment-weaver-app/src/main/resources/db/migration/V16__heartcove.sql
-- Spring Boot 启动时会自动跑。
--
-- 只有以下场景才需要手动跑本脚本：
--   - 不重启 Spring Boot，想临时建表验证
--   - 跨项目复用本模块（不依赖 moment-weaver-app 启动）
-- ============================================================
...
DROP TABLE IF EXISTS `heartcove_consent`;
CREATE TABLE `heartcove_consent` (
  `id`                 BIGINT       NOT NULL,
...
DROP TABLE IF EXISTS `heartcove_session`;
CREATE TABLE `heartcove_session` (
  `id`              BIGINT       NOT NULL,
  `subject_id`      BIGINT       NOT NULL                  COMMENT '关联 subject.id',
...
DROP TABLE IF EXISTS `heartcove_message`;
CREATE TABLE `heartcove_message` (
  `id`                   BIGINT       NOT NULL,
...
DROP TABLE IF EXISTS `heartcove_memory_summary`;
CREATE TABLE `heartcove_memory_summary` (
  `id`              BIGINT       NOT NULL,
...
DROP TABLE IF EXISTS `heartcove_audit_log`;
CREATE TABLE `heartcove_audit_log` (
  `id`           BIGINT       NOT NULL,
  `subject_id`   BIGINT       NOT NULL,
  `user_id`      BIGINT       NOT NULL,
